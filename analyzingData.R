...
averagePaymentPlot <- vendorTop10 %>% 
  ggplot(aes(reorder(glvendor,averagePayment), averagePayment)) + 
  geom_bar(stat='identity') + 
  coord_flip() + 
  ggtitle("Average Payment Per Vendor") +
  xlab("Vendor") +
...
explanationTop10Plot <- explanationTop10 %>%
  ggplot(aes(reorder(glexplanation, averagePayment), averagePayment)) +
  geom_bar(stat='identity') + 
  coord_flip() + 
  ggtitle("Average Payments by Explanation") +
  xlab("Explanation") +
  ylab("Average Payments") +
  theme(plot.title = 
          element_text(
...
# Top Explanations for 2015 + 2015
explanationSummary1516 <- data %>% 
  filter(grepl('energy purchases', glexplanation) == FALSE 
         & is.na(glexplanation) == FALSE
         & grepl('2015|2016', year)) %>% 
  group_by(year, glexplanation) %>%
  summarize(
    payments = sum(glamount), 
    count = length(glexplanation),
    averagePayment = 
...
explanationTop10 <- top_n(explanationSummary1516, 50, payments)

explanationSummary15Plot <- 
  explanationTop10 %>%
  filter(year == 2015) %>% 
  ggplot(aes(reorder(glexplanation,payments), payments)) +
  geom_bar(stat='identity') +
  coord_flip() + 
  ggtitle("2015 Payments by Explanation") +
  xlab("Explanation") +
  ylab("Payments") +
  theme(plot.title = 
          element_text(
